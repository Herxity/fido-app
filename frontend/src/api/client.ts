...
export const api = {
  ...
  createInquiry: async (): Promise<{ verificationSessionId: string; clientSecret: string }> => {
    const raw = await request<Wire>("/identity/inquiries", { method: "POST" });
    return { verificationSessionId: textValue(raw.verification_session_id), clientSecret: textValue(raw.client_secret) };
  },
